...
$result = mysqli_query($handle, "SELECT * FROM  booking where status='Rejected'");
$result2 = mysqli_query($handle, "SELECT * FROM workgroup WHERE wusername = '{$_SESSION['worker']}'");
...
echo '<table border="0" cellspacing="2" cellpadding="2"> 
  ...
  while ($row = $result->fetch_assoc()) 
  {
      ...
      echo '<tr> 
                ...
                <td>'.$field3name.'</td> 
                <td>'.$field4name.'</td>';
                echo "<td><a class='bt' href='workother.php?acpt=".$row["bookid"]."'>Accept</a></td>";
                echo "</tr>";
...
while ($row = $result2->fetch_assoc())
 {
    $field = $row["wid"];
    $field1 = $row["wusername"];

}
if(isset($_GET["acpt"]))
{
  $username = $_GET["acpt"];
  if($handle->query("UPDATE booking SET wid='$field', wname='$field1',status='Accepted' WHERE bookid='$field0name'"))
  {
