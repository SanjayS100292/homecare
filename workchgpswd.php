...
<body>
<form ation="workchgpswd.php" method="post">
...
function matchPassword() {  
  ...
  else
  {
    ...
    if (isset($_POST['chpassd'])) {
        $sql = "UPDATE workgroup SET wpassword='$_POST[npsw]' WHERE wusername='{$_SESSION['worker']}'";
        if ($handle->query($sql) == 1){
            ?>
            alert("Service provider password changed successfully..");
            <?php
