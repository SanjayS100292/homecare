...
echo '<table border="0" cellspacing="2" cellpadding="2"> 
  ...
  while ($row = $result->fetch_assoc()) 
  {
      ...
      echo '<tr> 
                ...
                echo "<td><a class='bt' href='adminviewworkers.php?del=".$row["wusername"]."'>Delete</a></td>";
                echo "</tr>";
}
...
{
  $username = $_GET["del"];
  if($handle->query("DELETE FROM workgroup WHERE wusername='$username'"))
  {
    print"<script>alert('Successfully Deleted');
    window.location.href= 'adminviewworkers.php'
    </script>";
  } else 
  { 
